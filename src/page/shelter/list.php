<?php
    ...
    $r = $control->model->getListQuery("select kode, longitude, latitude, keterangan from waypoint WHERE jenis='shelter' ORDER BY id DESC",['kode','longitude','latitude','keterangan']);
    $content = "<button class='btn btn-primary' onclick=\"createShelter = true;
                                                          callSingle('shelter', 'form.php', '', 'formShelter');\">Buat Point</button><hr>
    ...
    for($i=0;$i<count($r);$i++){
        ...
        $content .= "<li style='border-bottom: 1px dotted #ccc;'>".$control->view->getButtonDelete($dir, $r[$i]->kode)."  
                         ...
                         <div style='display: inline; font-size: 0.9em; padding-left: 15px;'><strong>".$r[$i]->kode."</strong> | ".$r[$i]->keterangan." | Link : ". implode(", ", $listLink)."</div></li>";
    }
    $content .= "</ol>";
